...
namespace Utils
{
	...
	inline void File::load_tree(AvlTree& tree, std::string file_path, int type)
	{
		enum Language
		{
			ENGLISH = 1,
	...
	inline void File::save_tree(AvlTree& tree, std::string file_path)
	{
		...
		tree.for_each_inorder([&](AvlNode* node)
			{
				if (file.is_open())
				{
					file << node->english_word().data() << "," << node->spanish_word().data() << "\n";
				};
			});
		file.close();
	}
